...
仓库分工：本公开 `RP-Hub` 是唯一正式网站代码与 GitHub Pages 发布源；私有 `RP-Hub-Sync` 保存过渡期用户脚本、加密同步数据、补丁规范源与迁移文档；私有 `RP-Hub-Card` 提供独立 LAN 角色卡 API 并保存旧站历史备份；私有且已归档的 `RP-Hub-Legacy` 只供历史回溯，Pages 和旧上游同步工作流已关闭。详见[维护说明](MAINTENANCE.md)。
...
`RP-Hub-Card` 是另一个局域网资源服务仓库，不在本网站的构建链内。同步用户脚本也不包含在本仓库；更新网站不会自动更新浏览器里已安装的脚本。正式域名经 Cloudflare 提供 HTTPS 和 HTTP 跳转；GitHub Pages 自身的“强制 HTTPS”开关目前因证书未就绪而无法启用，这不影响当前域名的 HTTPS 访问。完整文件差异与发布边界见[维护说明](MAINTENANCE.md)。
