...
GitHub Pages 已绑定正式域名；2026-09-25 的隔离 Chrome 验证首页正常挂载，网关和原生同步设置面板均出现。旧用户脚本在隔离环境注入后，网页仍可挂载。现有用户数据、实际设备和模型真实流式调用仍需验收；在完成迁移前保留已安装脚本与远端快照。

## 三个仓库的边界

| 仓库 | 职责 |
| --- | --- |
| `RP-Hub` | 干净历史的正式定制站点；`main` 发布 `rp.blycr.xyz` |
| `RP-Hub-Legacy` | 旧历史 fork，GitHub Pages 与上游同步工作流均已停用；等待脱离 fork 网络后私有归档 |
