    ...
    setup() {
        ...
        window.addEventListener('message', (event) => {
            ...
            if (!data || data.type !== 'RPHUB_PLAZA_CARD'
                || typeof data.cardId !== 'string' || !data.cardId.trim() || data.cardId.length > 128) return;
        ...
        const gatewayStatus = ref(window.RPHubApiClient.getGatewayStatus());
        const gatewayUrlDraft = ref(gatewayStatus.value.url);
        const gatewayOriginsDraft = ref(gatewayStatus.value.providerOrigins.join(', '));
        const gatewayTokenDraft = ref('');
        const saveGatewayConfig = () => {
            try {
                gatewayStatus.value = window.RPHubApiClient.configureGateway({
                    url: gatewayUrlDraft.value,
                    providerOrigins: gatewayOriginsDraft.value,
                    token: gatewayTokenDraft.value,
                });
                gatewayTokenDraft.value = '';
                showToast('网关已在本标签页启用', 'success');
            } catch (error) {
                showToast(error.message || '网关配置无效', 'error');
            }
        };
        const disableGateway = () => {
            window.RPHubApiClient.clearGatewayConfig();
            gatewayStatus.value = window.RPHubApiClient.getGatewayStatus();
            gatewayTokenDraft.value = '';
            showToast('网关已停用', 'success');
        };
        ...
        const migrateLegacyCustomApiProviders = () => {
            ...
            const referencedProviderIds = new Set([
                ...
                settings.visionModelProviderId,
            ].filter(Boolean));
        ...
        const getProviderReferences = (providerId) => {
            ...
            const settingsBindings = [
                ...
                ['visionModelProviderId', 'visionModel', '识图模型'],
            ];
        ...
        // RP-Hub-Sync upstream 1.9 embedded bridge
        ...
        const getNovelFrame = () => {
            const bySrc = document.querySelector('iframe[src*="novel/index.html"]');
            if (bySrc) return bySrc;
            return Array.from(document.querySelectorAll('iframe')).find((frame) => {
                try {
                    return /novel\/index\.html/i.test(frame.getAttribute('src') || frame.src || '');
                } catch (_) {
                    return false;
                }
            }) || null;
        ...
        const buildWorkshopModelBinding = (modelKey, providerKey) => {
            ...
            try {
                if (typeof getProviderApiConfig === 'function') {
        ...
        window.addEventListener('message', async (event) => {
            ...
            if (event.data && event.data.type === 'WORKSHOP_READY') {
                const workshopFrame = getWorkshopFrame();
                if (!workshopFrame || event.source !== workshopFrame.contentWindow
                    || event.origin !== getEmbeddedFrameOrigin(workshopFrame)) return;
            ...
            if (event.data?.type === 'WORKSHOP_IMPORT_AND_PLAY') {
                ...
                if (!iframe || event.source !== iframe.contentWindow
                    || event.origin !== getEmbeddedFrameOrigin(iframe) || workshopImportPending) return;
                ...
                try {
                    ...
                    event.source.postMessage({ type: 'WORKSHOP_IMPORT_RESULT', error: error.message || '导入失败，请重试' }, event.origin);
            ...
            if (event.data?.type === 'REQUEST_RPHUB_API_SETTINGS') {
                const iframe = getNovelFrame();
                if (!iframe || event.source !== iframe.contentWindow
                    || event.origin !== getEmbeddedFrameOrigin(iframe)) return;
                const mapNovelProvider = (provider) => {
                    const config = getProviderApiConfig(provider.id, { allowIncomplete: true });
                    return { id: provider.id, name: provider.name, icon: provider.icon || '',
                        apiUrl: config.apiUrl, apiKey: config.apiKey,
                        chatProtocol: config.chatProtocol };
                };
                event.source.postMessage({
                    type: 'RPHUB_API_SETTINGS', requestId: event.data.requestId,
                    settings: { apiProviderId: settings.apiProviderId,
                        apiProviderKeys: JSON.parse(JSON.stringify(settings.apiProviderKeys || {})),
                        apiProviderModes: JSON.parse(JSON.stringify(settings.apiProviderModes || {})),
                        apiKey: settings.apiKey },
                    providers: allApiProviderOptions.value.map(mapNovelProvider)
                }, event.origin);
            }
        });
        ...
        const chatModelSlots = computed(() => [
            { mode: 'quality', model: settings.qualityModel, providerId: settings.qualityModelProviderId || settings.apiProviderId },
            { mode: 'balanced', model: settings.balancedModel, providerId: settings.balancedModelProviderId || settings.apiProviderId },
            { mode: 'fast', model: settings.fastModel, providerId: settings.fastModelProviderId || settings.apiProviderId }
        ]);
        ...
        const normalizeMemorySettings = () => {
            ...
            const fields = new Set(['enabled', 'mode', 'embeddingModel', 'classicModel', 'embeddingProviderId', 'classicProviderId', 'summaryKeepFloors', 'classicConcurrency']);
        ...
        // RP-Hub-Sync upstream 1.9 provider routing
        const requestTrackedChatCompletion = (options, type) => {
            const providerId = options.providerId || (type === 'summary'
                ? memorySettings.classicProviderId
                : type === 'ui_template' ? settings.uiTemplateModelProviderId
                    : type === 'image_recognition' ? settings.visionModelProviderId
                        : settings.modelProviderId) || settings.apiProviderId;
            const config = getProviderApiConfig(providerId);
            const { providerId: _providerId, ...requestOptions } = options;
            const request = {
                ...requestOptions,
                url: buildApiEndpoint(config.apiUrl, config.chatProtocol === CHAT_PROTOCOL_OPENAI_RESPONSES
                    ? 'responses' : 'chat/completions'),
                apiKey: config.apiKey,
                protocol: config.chatProtocol,
            };
            return requestChatCompletion({ ...request, onUsage: (usage, metrics) => recordApiUsage(usage, {
                type, model: request.model, apiUrl: config.apiUrl, apiKey: config.apiKey, ...metrics
            }) });
        };
        const {
        ...
        const fetchModels = async (isManual = false) => {
            ...
            const results = await Promise.allSettled(configuredProviders.map(async config => {
                ...
                const response = await window.RPHubApiClient.fetchModelApi(request.url, request.init);
        ...
        const selectQuickModels = (slots) => {
            const previousModel = settings.model;
            const previousProviderId = settings.modelProviderId;
            ['quality', 'balanced', 'fast'].forEach((mode, index) => {
                const slot = slots[index] || {};
                const modelKey = getModelModeKey(mode);
                settings[modelKey] = String(slot.model || '').trim();
                settings[`${modelKey}ProviderId`] = slot.providerId || settings.apiProviderId;
            });
            ...
            settings.model = activeSlot?.model || previousModel;
            settings.modelProviderId = activeSlot?.providerId || previousProviderId;
        };
        ...
        const requestChatImageSelection = (input) => {
            // RP-Hub-Sync vision provider selection guard
            let visionConfig;
            try { visionConfig = getProviderApiConfig(settings.visionModelProviderId || settings.apiProviderId, { allowIncomplete: true }); }
            catch (_) { visionConfig = null; }
            if (!visionConfig?.apiUrl || !visionConfig?.apiKey || !settings.visionModel) {
        ...
        const requestClassicMemoryCompletion = async (requestMessages, signal) => {
            ...
            getProviderApiConfig(memorySettings.classicProviderId || settings.apiProviderId);
        ...
        const requestMemoryEmbeddings = async (inputs, signal, model = getMemoryEmbeddingModel()) => {
            const embeddingConfig = getProviderApiConfig(memorySettings.embeddingProviderId || settings.apiProviderId);
            if (embeddingConfig.embeddingProtocol !== EMBEDDING_PROTOCOL_OPENAI) throw new Error('所选供应商未启用向量能力');
            ...
            const apiUrl = embeddingConfig.apiUrl;
        ...
        const getEmbeddingApiUrl = () => getProviderApiConfig(memorySettings.embeddingProviderId || settings.apiProviderId, { allowIncomplete: true }).apiUrl;
        const hasCurrentSummaryEmbedding = (memory, model = getMemoryEmbeddingModel(), apiUrl = getEmbeddingApiUrl()) =>
        ...
        watch(() => [memorySettings.enabled, memorySettings.mode, memorySettings.classicModel, memorySettings.embeddingModel, memorySettings.classicProviderId, memorySettings.embeddingProviderId, settings.apiUrl], () => {
            abortClassicBatchExtraction();
        });
        ...
        const importCharacterData = async (rawData, avatarUrl, { askImageGeneration = true, activate = true } = {}) => {
            ...
            if (pending && pending.cardId) {
                const importedName = typeof char.name === 'string' ? char.name : '';
                const matchByName = pending.name === importedName;
                const isRecent = Number.isFinite(pending.receivedAt) && Date.now() - pending.receivedAt < 120000;
                if (matchByName && isRecent) {
        ...
        return {
            ...
            user, settings, gatewayStatus, gatewayUrlDraft, gatewayOriginsDraft, gatewayTokenDraft, saveGatewayConfig, disableGateway,
...
app.config.compilerOptions.isCustomElement = tag => tag === 'rphub-native-sync';
