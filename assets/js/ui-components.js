...
(function () {
    ...
    const ModelSelectorModal = {
        props: {
            ...
            slotProviderIds: { type: Array, default: () => [] },
            currentProviderId: { type: String, default: '' },
            providerDisplayName: { type: Function, default: null },
            manualModelProviderId: { type: String, default: '' },
            manualModelId: { type: String, default: '' },
            manualModelProviderOptions: { type: Array, default: () => [] },
            isModelSelected: { type: Function, default: null }
        },
        emits: ['close', 'select', 'select-slots', 'select-manual', 'refresh', 'update:search-query', 'update:active-tag', 'update:manual-model-provider-id', 'update:manual-model-id'],
        data() {
            return {
                activeSlot: 0,
                draftSlotModels: ['', '', ''],
                draftSlotProviderIds: ['', '', '']
            };
        },
        computed: {
            selectedModel() {
        ...
        watch: {
            show(visible) {
                if (visible && this.target === 'quickModels') {
                    ...
                    this.draftSlotProviderIds = [0, 1, 2].map(index => this.slotProviderIds[index] || '');
                }
            }
        },
        methods: {
            selected(model) {
                if (this.target === 'quickModels') return this.draftSlotModels[this.activeSlot] === model.id
                    && this.draftSlotProviderIds[this.activeSlot] === model.providerId;
                return this.isModelSelected?.(model) || (this.currentModel === model.id
                    && (!this.currentProviderId || this.currentProviderId === model.providerId));
            },
            selectManual() {
                if (this.target === 'quickModels') {
                    if (this.manualModelId && this.manualModelProviderId)
                        this.chooseModel(this.manualModelId.trim(), this.manualModelProviderId);
                    return;
                }
                this.$emit('select-manual');
            },
            selectionClass(active) {
                return active ? 'border-primary-400 bg-primary-50 text-primary-700'
                    : 'border-gray-200 bg-white text-gray-600 hover:bg-gray-50';
            },
            chooseModel(modelId, providerId) {
                if (this.target !== 'quickModels') {
                    this.$emit('select', modelId, providerId);
                    return;
                }
                const selected = this.draftSlotModels[this.activeSlot] === modelId
                    && this.draftSlotProviderIds[this.activeSlot] === providerId;
                this.draftSlotModels[this.activeSlot] = selected ? '' : modelId;
                this.draftSlotProviderIds[this.activeSlot] = selected ? '' : providerId;
                this.$emit('select-slots', this.draftSlotModels.map((model, index) => ({
                    model, providerId: this.draftSlotProviderIds[index]
                })));
            }
        },
        template: `
            <transition name="fade">
                <modal-shell v-if="show" overlay-class="z-50 bg-black/50 backdrop-blur-sm p-3 sm:p-4"
                    ...
                    <section class="flex min-h-0 flex-1 flex-col" role="dialog" aria-modal="true" aria-labelledby="model-selector-title"
                        ...
                        <!-- RP-Hub-Sync upstream 1.9 model selector -->
                        <div class="flex shrink-0 flex-col gap-3 px-4 py-3 sm:px-5">
                            <div class="flex flex-wrap items-center gap-2">
                                <custom-select :model-value="manualModelProviderId" :options="manualModelProviderOptions"
                                    placeholder="选择供应商" class="min-w-36 flex-1"
                                    @update:model-value="$emit('update:manual-model-provider-id', $event)"></custom-select>
                                <input :value="manualModelId" @input="$emit('update:manual-model-id', $event.target.value)"
                                    @keydown.enter.prevent="$emit('select-manual')" type="text" placeholder="手动输入模型 ID"
                                    class="min-w-36 flex-1 rounded-lg border border-gray-200 px-3 py-2 text-sm">
                                <button type="button" @click="$emit('select-manual')" class="rounded-lg bg-primary-600 px-3 py-2 text-sm text-white">使用</button>
                                <button type="button" @click="$emit('refresh')" class="rounded-lg border border-gray-200 px-3 py-2 text-sm">刷新列表</button>
                            </div>
                            <div class="relative">
                        ...
                        <div class="model-selector-list min-h-0 flex-1 overflow-y-auto overscroll-contain bg-gray-50/40 p-3 custom-scrollbar sm:p-4">
                            ...
                            <div class="grid grid-cols-1 gap-2 sm:grid-cols-2" role="group" aria-label="可选模型">
                                <button v-for="model in models" :key="(model.providerId || 'default') + ':' + model.id" type="button" @click="chooseModel(model.id, model.providerId)"
                                    :aria-label="model.id" :aria-pressed="selected(model)" :title="model.id"
                                    ...
                                    :class="selected(model) ? 'border-primary-400 bg-primary-50 text-primary-800' : 'border-gray-200 bg-white text-gray-800 hover:border-primary-200 hover:bg-primary-50/40'">
                                    <span class="min-w-0 flex-1 text-sm font-semibold leading-5 [overflow-wrap:anywhere]">{{ model.id }}<small v-if="model.providerId" class="block text-gray-500">{{ providerDisplayName?.(model.providerId) || model.providerId }}</small></span>
                                    <svg class="h-5 w-5 shrink-0 text-primary-600" :class="{ invisible: !selected(model) }"
