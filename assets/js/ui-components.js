...
(function () {
    ...
    const ModelSelectorModal = {
        ...
        emits: [
            ...
            'update:search-query',
            'update:active-tag',
            'update:manual-model-provider-id',
            'update:manual-model-id',
        ],
        methods: {
            selected(model) {
                if (typeof this.isModelSelected === 'function') return this.isModelSelected(model);
                return this.currentModel === model.id
                    && (!this.currentProviderId || !model.providerId || this.currentProviderId === model.providerId);
            },
            providerLabel(model) {
                if (!model?.providerId) return '';
                if (typeof this.providerDisplayName === 'function') return this.providerDisplayName(model.providerId);
        ...
        template: `
            <transition name="fade">
                <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div class="bg-white rounded-xl border border-gray-200 w-full max-w-2xl max-h-[90vh] h-[90vh] flex flex-col shadow-2xl transform transition-all scale-100">
                        <div class="p-4 border-b border-gray-100 flex justify-between items-center gap-3">
                            <h3 class="text-lg font-bold text-gray-800">选择模型</h3>
                            <div class="flex items-center gap-2">
                                <button v-if="showRefresh" type="button" @click="$emit('refresh')" title="刷新模型列表"
                                    class="text-gray-400 hover:text-primary-600 focus:outline-none transition-colors p-1"
                                    :disabled="refreshing">
                        ...
                        <div class="p-4 border-b border-gray-100 flex flex-col gap-3">
                            <!-- RP-Hub-Sync manual provider model selection v4 -->
                            <div class="flex flex-col sm:flex-row gap-2 sm:items-center">
                                <custom-select
                                    :model-value="manualModelProviderId"
                                    :options="manualModelProviderOptions"
                                    placeholder="选择供应商"
                                    class="w-full sm:w-44 shrink-0"
                                    @update:model-value="$emit('update:manual-model-provider-id', $event)">
                                </custom-select>
                                <input :value="manualModelId" @input="$emit('update:manual-model-id', $event.target.value)"
                                    type="text" placeholder="手动输入模型 ID"
                                    @keydown.enter.prevent="$emit('select-manual')"
                                    class="min-w-0 w-full h-10 border rounded-lg px-3 py-2 bg-gray-50/60 border-gray-300 font-mono text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all shadow-sm">
                                <button type="button" @click="$emit('select-manual')"
                                    class="w-full sm:w-auto min-w-[4.5rem] h-10 px-4 inline-flex items-center justify-center rounded-lg bg-primary-600 text-white text-sm font-bold hover:bg-primary-700 active:scale-95 transition-all shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500/30">
                                    使用
                                </button>
                            </div>
                            <input :value="searchQuery" @input="$emit('update:search-query', $event.target.value)" type="text"
                                placeholder="检索模型..."
                                class="w-full border rounded-lg px-4 py-2 focus:outline-none transition-all shadow-sm bg-gray-50/60 border-gray-300 text-gray-800 focus:ring-2 focus:ring-primary-500 focus:shadow-md">
                        ...
                        <div class="flex-1 overflow-y-auto p-2 min-h-[300px]">
                            ...
                            <div class="space-y-1">
                                <button v-for="model in models"
                                    :key="(model.providerId || 'default') + ':' + model.id"
                                    @click="$emit('select', model.id, model.providerId)"
                                    class="w-full text-left px-4 py-3 rounded-xl hover:bg-gray-50 hover:shadow-[0_2px_4px_rgba(0,0,0,0.02)] transition-colors flex justify-between items-center gap-3 group border border-transparent hover:border-gray-100 active:bg-gray-100">
                                    <span class="min-w-0 flex flex-col gap-0.5">
                                        <span class="text-gray-700 font-mono font-medium break-all group-hover:text-primary-600 transition-colors">{{ model.id }}</span>
                                        <span v-if="model.providerId" class="text-xs text-gray-400 truncate">{{ providerLabel(model) }}</span>
                                    </span>
                                    <span v-if="selected(model)" class="shrink-0 text-primary-600 bg-primary-50 p-1 rounded-full shadow-sm">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                        </svg>
                                    </span>
